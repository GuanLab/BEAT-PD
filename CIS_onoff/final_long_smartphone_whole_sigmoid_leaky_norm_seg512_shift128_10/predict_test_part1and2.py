...
if __name__ == '__main__':
    model = 'raye_version2_2000_deeper'
    params = 'params'

    import pkgutil
    loader = pkgutil.get_importer('/ssd/gyuanfan/PD_walking_project/regression_Part_1_and_2_score_fortransfer/model')
    ...
    net, _, _ = model.network(input_var, label_var, shape)
...
## get all 500 sequences;
        ...
        the_count=0
        vector=np.zeros(31)
        while ((i<len(image)-2000)):
            data=image[i:(i+2000),:]
            data_mean=np.mean(data,axis=0)
            data=(data-data_mean)/np.std(data,axis=0)
            sub_data=np.zeros((2000,3))
